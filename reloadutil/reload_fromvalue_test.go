package reloadutil

import (
	"crypto/x509"
	"errors"
	"io/ioutil"
	"testing"
)

var (
	password = "password"
	cert     = []byte(`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`)
	key = []byte(`-----BEGIN RSA PRIVATE KEY-----
Proc-Type: 4,ENCRYPTED
DEK-Info: DES-EDE3-CBC,64B032D83BD6A6DC


-----END RSA PRIVATE KEY-----`)
)

func TestValueCertificateGetterReloadFunc(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "vault-test")
	if err != nil {
		t.Fatalf("Error creating temporary directory: %s", err)
	}
	certFile := tempDir + "/server.crt"

	err = ioutil.WriteFile(certFile, cert, 0755)
	if err != nil {
		t.Fatalf("error writing to temp file: %s", err)
	}

	vcg, err := NewValueCertificateGetter(certFile, key, []byte(password))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	err = vcg.Reload()
	if err == nil {
		t.Fatalf("expected error, got nothing")
	}
}

func TestValueCertificateGetterKeyWithPassphrase(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "vault-test")
	if err != nil {
		t.Fatalf("Error creating temporary directory: %s", err)
	}
	certFile := tempDir + "/server.crt"

	err = ioutil.WriteFile(certFile, cert, 0755)
	if err != nil {
		t.Fatalf("Error writing to temp file: %s", err)
	}

	_, err = NewValueCertificateGetter(certFile, key, []byte(""))
	if !errors.As(err, &x509.IncorrectPasswordError) {
		t.Fatalf("expected incorrect password error, got %v", err)
	}

	vcg, err := NewValueCertificateGetter(certFile, key, []byte(password))
	if err != nil {
		t.Fatalf("unexpected error %v", err)
	}
	if vcg.c == nil {
		t.Fatalf("expected tls certificate to exist")
	}
}
